...
export class SupabaseStore {
  ...
  async updateEntry(id, patch) {
    ...
    // 只写调用方真正改了的字段。以前是把整行按读到的快照写回去，每次都要跨网络
    // 往返读一次；两个并发 PATCH（比如同时改数值和备注）各自基于旧快照写全字段，
    // 后落地的那个会把先落地的覆盖掉。SQLite 版因为 API 是同步的，反而没这问题。
    const body = {};
    if (patch.metricKey !== undefined) body.metric_id = entry.metricId;
    if (patch.value !== undefined) body.value = entry.value;
    if (patch.recordedOn !== undefined) body.recorded_on = entry.recordedOn;
    if (patch.note !== undefined) body.note = entry.note;
  ...
  async createTask(body) {
    const rows = await this.request('/rest/v1/tasks', {
      ...
      body: {
        ...
        due_on: normalizeDueOn(body.dueOn),
  ...
  async updateTask(id, patch) {
    ...
    // 同上：只写改过的字段。特别是 completed_at —— 只有 patch 里带了 status 才写，
    // 否则并发改标题的那次请求会把 status 和 completed_at 一起按旧快照覆盖回去。
    const body = {};
    if (patch.title !== undefined) body.title = title;
    if (patch.domain !== undefined) body.domain = domain;
    if (patch.priority !== undefined) body.priority = priority;
    if (patch.dueOn !== undefined) body.due_on = dueOn;
  ...
  // 同 sqlite-store：报告列表 LIMIT 20 且无分页，误生成的重复报告删不掉就永久占名额。
  async deleteReport(id) {
    const rows = await this.request(`/rest/v1/reports?id=eq.${encodeURIComponent(id)}`, {
      method: 'DELETE', headers: { prefer: 'return=representation' }
    });
    return rows.length > 0;
  }
  ...
  // 展开哪一份再单独取。payload_json 是 jsonb，取回来已经是对象。
  async reportById(id) {
    const rows = await this.request(`/rest/v1/reports?select=id,period,title,summary,payload_json,created_at&id=eq.${encodeURIComponent(id)}&limit=1`);
