...
export class SupabaseStore {
  ...
  async updateTask(id, status) {
    const rows = await this.request(`/rest/v1/tasks?id=eq.${encodeURIComponent(id)}`, {
      method: 'PATCH',
      headers: { prefer: 'return=representation' },
      body: { status, completed_at: status === 'done' ? new Date().toISOString() : null }
    });
    return rows.length > 0;
  }
  ...
  async uploadImage({ bytes, originalName, mimeType, category, relatedType, relatedId, capturedAt, note }) {
    ...
    const extension = extensions[mimeType] || path.extname(originalName).toLowerCase().replace(/[^.a-z0-9]/g, '').slice(0, 8) || '.bin';
    const now = new Date();
    const objectPath = `${now.getUTCFullYear()}/${String(now.getUTCMonth() + 1).padStart(2, '0')}/${randomUUID()}${extension}`;
